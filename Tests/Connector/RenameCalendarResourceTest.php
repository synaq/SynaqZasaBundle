...
class RenameCalendarResourceTest extends ZimbraConnectorTestCase
{
    /**
     * @var ZimbraConnector
     */
    ...
    {
        $this->connector->renameCalendarResource('WHAT-EVER', 'foo@foo.com');
     ...
     * @throws SoapFaultException
     */
    public function renamesForTheGivenId()
    {
        $this->connector->renameCalendarResource('SOME-ID', 'foo@foo.com');
        $this->client->shouldHaveReceived('post')->with(m::any(), m::on(function ($body) {
            return preg_match('/RenameCalendarResourceRequest.*id="SOME-ID"/', $body) === 1;
        }), m::any(), m::any(), m::any());
    }
    ...
    {
        $this->connector->renameCalendarResource('ANY-OLD-ID', 'foo@foo.com');
        $this->client->shouldHaveReceived('post')->with(m::any(), m::on(function ($body) {
            return preg_match('/RenameCalendarResourceRequest.*id="ANY-OLD-ID"/', $body) === 1;
        }), m::any(), m::any(), m::any());
    }
     ...
     * @throws SoapFaultException
     */
    public function sendsTheGivenNewName()
    {
        $this->connector->renameCalendarResource('WHAT-EVER', 'foo@bar.com');
        $this->client->shouldHaveReceived('post')->with(m::any(), m::on(function ($body) {
            return preg_match('/RenameCalendarResourceRequest.*newName="foo@bar.com"/', $body) === 1;
        }), m::any(), m::any(), m::any());
    }
    ...
    {
        $response = $this->connector->renameCalendarResource('WHAT-EVER', 'foo@bar.com');
    ...
    {
        $message = '<RenameCalendarResourceResponse xmlns="urn:zimbraAdmin">
  <calresource name="foo@bar.com" id="SOME-ID">
    ...
    <a n="zimbraCreateTimestamp">20200817090615Z</a>
    ...
    <a n="zimbraMailDeliveryAddress">foo@bar.com</a>
    ...
    <a n="mail">rename.test.1@synaq.com</a>
    ...
    <a n="zimbraMailTransport">lmtp:some.host.com:7025</a>
    ...
    <a n="zimbraPasswordModifiedTime">20200817090615Z</a>
